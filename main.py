...
def run_training():
    ...
    for epoch in range(training_config['num_epochs']):
        ...
        # Save the top 2 best checkpoints based on validation accuracy
        if len(best_checkpoints) < 2 or val_acc > best_checkpoints[-1][0]:
            best_checkpoints.append((val_acc, copy.deepcopy(model.state_dict())))
            best_checkpoints = sorted(best_checkpoints, key=lambda x: x[0], reverse=True)[:2]

            # Save the best checkpoints
            for idx, (acc, state) in enumerate(best_checkpoints, 1):
                checkpoint_path = os.path.join(checkpoint_dir, "best_{idx}.pth")
